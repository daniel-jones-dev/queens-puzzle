...
impl QueensPuzzle {
    pub fn new(region_vecs: Vec<Vec<Cell>>) -> Self {
    ...
    /// Creates an empty n×n puzzle with n unassigned regions (for editor mode).
    pub fn new_empty(n: usize) -> Self {
        let board = Grid::new(n, n);
        let cell_regions = Grid::new(n, n);
        let regions = (0..n).map(|_| HashSet::new()).collect();
        Self {
            board,
            cell_regions,
            regions,
    ...
    pub fn cell_state(&self, cell: Cell) -> State {
        self.board[cell]
    }

    /// Sets the state of a cell
    pub fn set_cell_state(&mut self, cell: Cell, state: State) {
        self.board[cell] = state;
    }

    /// Assigns a region to a cell (silently overwrites any existing assignment)
    pub fn set_cell_region(&mut self, cell: Cell, region: Option<u8>) {
        if let Some(old_region) = self.cell_regions[cell] {
            self.regions[old_region as usize].remove(&cell);
        }
        self.cell_regions[cell] = region;
        if let Some(r) = region {
            self.regions[r as usize].insert(cell);
        }
    }
    ...
    pub fn all_regions_iter(&self) -> impl Iterator<Item = (HashSet<Cell>, usize)> + '_ {
        self.regions
            .iter()
            .enumerate()
    ...
    /// Returns all queens that are in conflict (same row, column, region, or diagonally adjacent).
    pub fn clashing_queens(&self) -> Vec<Cell> {
        let queens = self.queens();
        let mut clashing = std::collections::HashSet::new();
        for &q in &queens {
            for &other in &queens {
                if q == other {
                    continue;
                }
                let conflicts = q.row == other.row
                    || q.col == other.col
                    || self.cell_regions[q] == self.cell_regions[other]
                        && self.cell_regions[q].is_some()
                    || (q.row as i32 - other.row as i32).abs() == 1
                        && (q.col as i32 - other.col as i32).abs() == 1;
                if conflicts {
                    clashing.insert(q);
                }
            }
        }
        clashing.into_iter().collect()
    }
}
