...
pub struct HiddenSet { // Naming comes from sudokuwiki.org "Hidden Pair"
    pub(crate) n: usize,
}
            ...
            match unknowns {
                ...
                Some((unknowns_cells, is_col, row_or_col_indices)) => {
                    ...
                    let regions_group = format!("The {} regions",
                                                solver::oxford_comma(regions_sorted.map(|region| {region_color_name(region)})));
                    ...
                    let row_or_col_group = format!("{row_or_cols} {}",
                                                   solver::oxford_comma(row_or_col_indices_sorted.map(|index| {if is_col {column_name(index)} else {row_name(index)}})));
